#!/usr/bin/env python3
"""drill_authoff.py — store gate 1 auth-off regression drill ( pattern).

Boots with NO JANUS_MASTER_KEY resolved (the runner exports nothing) and
config.memory.toml (no [janus.store] section ⇒ in-memory default; [janus.keys] auth = "off" ⇒
auth off) — the the earlier smoke gates byte-identical default. Asserts:
