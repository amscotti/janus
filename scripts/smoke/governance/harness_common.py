...
def _is_series_line(line: str, name: str) -> bool:
    ...
    immediately followed by '{' (labeled), whitespace (unlabeled), or EOL — never
    a sibling series that merely starts with the name (histogram ``_bucket`` /
    ``_sum`` / ``_count`` lines would otherwise be summed into the base series)."""
