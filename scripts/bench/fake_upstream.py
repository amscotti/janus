...
budget/cost legs — fixtures stay the
committed corpus, read-only):
  --usage-override  JSON {"prompt_tokens": N, "completion_tokens": M} — pins the
                     response token counts deterministically (the budget exact-cap legs
                     need totals other than the corpus 14/12). Applied to the
                     non-stream golden body's usage object and to the stream's terminal
                     usage frame.
  usage-frame gating — mirrors real OpenAI: a stream whose request does NOT carry
                     stream_options.include_usage drops the terminal usage frame (the
                     Byte-golden wire discipline: Janus never forces include_usage upstream), so
                     the gateway settles the documented $0 entry; with include_usage
                     requested the frame is served (overridden when --usage-override
                     is set).
...
class FakeUpstream:
    ...
    def __init__(
        ...
        self.nonstream_body = self._load_nonstream_body(fixtures)
    ...
    def _load_nonstream_body(self, fixtures: Path) -> bytes:
        """The committed golden body; under --usage-override the usage object is
        replaced with the pinned counts (the fixtures themselves stay untouched)."""
        body = json.loads((fixtures / "chat.response.json").read_bytes())
        if self.usage_override is not None:
            body["usage"] = {
                "prompt_tokens": self.usage_override["prompt_tokens"],
                "completion_tokens": self.usage_override["completion_tokens"],
                "total_tokens": self.usage_override["prompt_tokens"] + self.usage_override["completion_tokens"],
            }
    ...
    def handle_chat(self, handler: BaseHTTPRequestHandler, raw_body: bytes) -> None:
        ...
        # stage 4: real-OpenAI usage-frame gating — the terminal usage frame is served
        # only when the client requested stream_options.include_usage (Janus never
        # forces it upstream — the byte-golden discipline).
        wants_usage = '"include_usage":true' in text or '"include_usage": true' in text
        ...
        if mode == "tools":
            ...
            if has_tool_result:
                if wants_stream or "stream=1" in text:
                    self._stream_plain(handler, pause, '"include_usage":true' in text or '"include_usage": true' in text)
                else:
        ...
        if mode == "stream" or is_stream:
            self._stream_plain(handler, pause, wants_usage)
            return
    ...
    def _stream_plain(self, handler: BaseHTTPRequestHandler, pause: bool, wants_usage: bool) -> None:
        ...
        for frame_index, payload in enumerate(self.stream_frames):
            is_usage_frame = '"usage"' in payload and '"choices":[]' in payload
            if is_usage_frame and not wants_usage:
                continue  # stage 4: real-OpenAI gating — no include_usage ⇒ no usage frame
            if is_usage_frame and self.usage_override is not None:
                usage = {
                    "prompt_tokens": self.usage_override["prompt_tokens"],
                    "completion_tokens": self.usage_override["completion_tokens"],
                    "total_tokens": self.usage_override["prompt_tokens"]
                    + self.usage_override["completion_tokens"],
                }
                try:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        / "janus-core/src/test/resources/fixtures/openai",
)
    ...
    parser.add_argument("--usage-override", type=str, default=None)
    args = parser.parse_args()

    override = None
    if args.usage_override is not None:
        try:
            override = json.loads(args.usage_override)
        except json.JSONDecodeError as exc:
            raise SystemExit(f"--usage-override must be JSON: {exc}")
        if "prompt_tokens" not in override or "completion_tokens" not in override:
            raise SystemExit("--usage-override must carry prompt_tokens and completion_tokens")
